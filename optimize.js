(function () {
  ...
  function calcStickySize(baseW, baseH) {
    ...
    // Mobile: tối đa 70% chiều rộng màn hình
    // Desktop: dùng baseW, tối đa viewport - 24px
    const maxW = isMobile
      ? Math.round(window.innerWidth * 0.7)
      : Math.min(baseW, window.innerWidth - 24);
    const w = maxW;
    const h = Math.round(w * ratio); // height tự co theo tỷ lệ
    return { w, h };
  }
  ...
  function injectStyles() {
    ...
    st.textContent = `
      ...
      /* Outstream */
      .xad-outstream-wrap{min-width:300px;border-radius:0!important;background:transparent!important;}
      ...
      .xad-outstream-wrap .vjs-big-play-button,.xad-outstream-wrap .vjs-control-bar{display:none!important}
      ...
      /* ═══ TOPBAR — nằm NGOÀI khung, PHÍA TRÊN video ═══
         bottom:100% đẩy bar lên trên wrapper, không che video
      */
      .xad-topbar{
        position:absolute;
        bottom:100%;
      ...
      .xad-wrap.is-sticky .xad-topbar{opacity:1;pointer-events:auto}

      /* Nút X — góc trái, nền đen vuông */
      .xad-close{
      ...
      /* Brand label — góc phải, nền đen */
      .xad-brand{
        height:26px;
        font:800 11px/26px system-ui,sans-serif;
  ...
  /* ════════════════  Topbar (X + Brand) ════════════════
     Nằm NGOÀI khung video, phía TRÊN — dùng bottom:100% trong CSS
     Hiện khi sticky, ẩn khi normal
  */
  function addTopbar(wrapper, onClose, brand) {
    const bar = document.createElement("div");
    bar.className = "xad-topbar";
  ...
  function mountInstream(el) {
    ...
    const brand       = el.getAttribute("data-brand") || "BIGFOURTH";
  ...
  function mountOutstream(container) {
    ...
    player.ready(() => {
      ...
      player.on("ads-ad-started", () => {
        retrier.reset(); adCount++;
        if (debug) console.log("[XAD] ad #" + adCount + " started");
        if (sticky) sticky.forceSticky();
      });

      /* ── Events đúng của videojs-contrib-ads ──
       * adend     : ad kết thúc (đúng tên, không phải ads-ad-ended)
       * adskip    : user skip ad
       * adserror  : lỗi VAST / no fill
       * contentresumed: content resume sau ad break (fallback)
       *
       * Dùng debounce để tránh schedule 2 lần nếu nhiều event cùng fire
       */
      let _schedAt = 0;
      function trySchedule(evt) {
        const now = Date.now();
        if (now - _schedAt < 800) return;
        _schedAt = now;
